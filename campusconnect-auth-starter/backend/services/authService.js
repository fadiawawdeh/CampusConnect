...
const emailDomain = (process.env.UNIVERSITY_EMAIL_DOMAIN || '@isik.edu.tr').toLowerCase();

const normalizeEmail = (email) => email?.trim().toLowerCase();

const ensureUniversityEmail = (email) => {
  if (!email || !email.endsWith(emailDomain)) {
    throw new Error(`Only university emails ending with ${emailDomain} are allowed.`);
  }
};
